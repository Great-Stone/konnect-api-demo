...
def main():
    ...
    server = os.environ.get("KONNECT_SERVER_URL", "https://us.api.konghq.com").rstrip("/")
    base = f"{server}/v2/control-planes/{control_plane_id}/core-entities"

    routes = api_json("GET", f"{base}/routes?size=1000", token).get("data", [])
    ...
    for route_name, payload in route_plugin_payloads().items():
        ...
        existing_plugins = api_json("GET", f"{plugins_url}?size=1000", token).get("data", [])
        existing = next((plugin for plugin in existing_plugins if plugin.get("name") == "datakit"), None)
        if existing is None:
            created = api_json("POST", plugins_url, token, payload)
            configured.append({"route": route_name, "plugin_id": created["id"], "action": "created"})
        else:
            api_json("PUT", f"{plugins_url}/{existing['id']}", token, payload)
